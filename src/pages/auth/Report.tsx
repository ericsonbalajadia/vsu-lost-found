...
export default function Report() {
  ...
  // Predictive insights on building selection
  const handleBuildingChange = useCallback(async (b: string) => {
    setBuilding(b);
    if (b && b !== 'General Campus') {
  ...
  // Inside Report component, after location state
useEffect(() => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Redirect to item detail (Phase 4 will implement this page)
  ...
  return (
    <div className="min-h-screen bg-background text-on-surface overflow-hidden h-screen flex">
      {/* Right‑hand side content – scrollable left column + sticky right column */}
  ...
  />
              ...
              <section className="bg-surface-container-low p-8 rounded-xl relative overflow-hidden border border-outline-variant/10">
                ...
                <div className="space-y-8">
                  <div>
                    <label htmlFor="reportType" className="block text-sm font-medium text-on-surface-variant mb-2">Report Type</label>
                    <select
                      id="reportType"
                      value={type}
      ...
      <div className="fixed top-0 left-0 w-full h-full pointer-events-none -z-10 overflow-hidden">
        ...
        <div className="absolute bottom-[10%] -left-[5%] w-[40%] h-[40%] rounded-full bg-tertiary/5 blur-[100px]"></div>
      </div>
    </div>
  );
}