...
export default function AuthenticatedLayout({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-background">
      ...
      >
        <div
          className={`p-6 mb-2 border-b border-outline-variant/50 flex items-center gap-3 ${collapsed ? 'justify-center' : ''}`}
        >
          <img
            alt="FoundPath Logo"
            className={`object-contain ${collapsed ? 'h-12 w-12' : 'h-14 w-auto'}`}
            src="/FoundPath.png"
          />
          {!collapsed && (
        ...
        <div className="p-6">
          <nav
            className={`pb-6 border-t border-outline-variant/50 pt-6 ${collapsed ? 'px-0' : 'px-4'}`}
          >
            <ul className="space-y-1.5">
              <li>
                <Link
                  to="/settings/profile"
                  className={`flex items-center gap-3 text-on-surface-variant hover:text-primary hover:bg-primary/5 rounded-xl transition-all duration-200 font-headline font-semibold text-sm ${
                    collapsed ? 'justify-center py-3' : 'px-4 py-3'
                  }`}
                >
              ...
              <li>
                <button
                  onClick={signOut}
                  aria-label="Log Out"
                  className={`w-full flex items-center gap-3 hover:text-error hover:bg-error/5 rounded-xl transition-all duration-200 font-headline font-semibold text-sm text-error ${
                    collapsed ? 'justify-center py-3' : 'px-4 py-3'
                  }`}
                >
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-background overflow-hidden">
        <header className="h-16 bg-surface-container-lowest/80 backdrop-blur-md border-b border-outline-variant/10 sticky top-0 z-50 w-full flex items-center justify-between px-6 lg:px-10 shrink-0">
          <div className="flex items-center gap-2">
            ...
            <Link to="/inventory" className="md:hidden flex items-center gap-2">
              <img alt="FoundPath Logo" className="h-8 w-8 object-contain" src="/FoundPath.png" />
